    ...
    while (x_ini >= 0) and (y_ini >= 0) and (x_ini <= 7) and (y_ini <= 7):
        if (tabuleiro[y_ini][x_ini] == cor):
            ...
            if( y_ini + d_y > 7 or y_ini + d_y <0 or x_ini + d_x > 7 or x_ini + d_x <0):
                ...
                if limitada == "SEMIABERTA" and seq_cores == comprimento:
                ...
                if limitada == "ABERTA" and seq_cores == comprimento:
                    qtd_seq_aberta += 1
                if limitada == "SEMIABERTA" and seq_cores == comprimento:
                    qtd_seq_semiaberta += 1
                seq_cores=0
...
def detecta_linhas(tabuleiro, cor, comprimento):
    ...
    #percorre as diagonais do tipo 1,1
    for i in range(0, 7):
...
def busca_max(tabuleiro):
    ...
    for y in range(8):
        for x in range(8):
            if tabuleiro[y][x]==" ":
                tabuleiro[y][x] = "p"
                if pontuacao(tabuleiro) > melhor_pontuacao:
                    movimento_y = y
                    movimento_x = x
                    melhor_pontuacao = pontuacao(tabuleiro)
                tabuleiro[y][x] = " "
    if movimento_x<0:
        return
...
def sequencia_vitoria(tabuleiro,cor,y_ini,x_ini,d_y,d_x):
    seq_cores=0
    while (x_ini >= 0) and (y_ini >= 0) and (x_ini <= 7) and (y_ini <= 7):
        if (tabuleiro[y_ini][x_ini] == cor):
            seq_cores+=1
...
def sequencia_vitoriosa(tabuleiro,cor):
    vitoria = 0
    #percorre as colunas
    for i in range(0,7):
        vitoria += sequencia_vitoria(tabuleiro, cor, 0,i,1,0)
        #percerre as linhas
    for i in range(0, 7):
       vitoria += sequencia_vitoria(tabuleiro, cor, i,0,0,1)
    #percorre as diagonais do tipo 1,1
    for i in range(0, 7):
       vitoria += sequencia_vitoria(tabuleiro, cor, i,0,1,1)
      #na outra metade do tipo 1,1
    for i in range(1, 7):
        vitoria += sequencia_vitoria(tabuleiro, cor, 0,i,1,1)
    #percorre as diagonais do tipo 1,-1
    for i in range(0, 7):
       vitoria += sequencia_vitoria(tabuleiro, cor, 0,i,1,-1)
...
def e_vitoria(tabuleiro):
    ...
    if sequencia_vitoriosa(tabuleiro,"b")>0:
        return "Venceram as brancas"

    if sequencia_vitoriosa(tabuleiro,"p")>0:
